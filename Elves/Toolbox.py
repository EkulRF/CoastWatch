...
def image_retrieval(point,dates, sat_list):
    
    Sat = []

    if 'L5' in sat_list:
        Landsat5 = ee.ImageCollection("LANDSAT/LT05/C01/T1_TOA").filterBounds(point).filterDate(dates[0], dates[1])
        Sat.append(Landsat5)
    if 'L8' in sat_list:
        Landsat8 = ee.ImageCollection('LANDSAT/LC08/C01/T1_TOA').filterBounds(point).filterDate(dates[0], dates[1])
        Sat.append(Landsat8)
    if 'S2' in sat_list:
        Sentinel2 = ee.ImageCollection("COPERNICUS/S2").filterBounds(point).filterDate(dates[0], dates[1]).filter(ee.Filter.lte('CLOUDY_PIXEL_PERCENTAGE', 80))
